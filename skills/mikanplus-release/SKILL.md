...
- CI derives the artifact version from the tag, but the packaging defaults still need to match the workspace version for local builds.
...
4. Update only the release version locations listed above. Do not globally replace dependency versions in `Cargo.lock`.
5. Run `cargo check --workspace --all-targets` once to synchronize local workspace package entries in `Cargo.lock`, then verify the diff.
6. Run the release validation below. Keep the version bump uncommitted when the user says “prepare” or otherwise has not asked to commit it.

For a normal patch release, the expected version diff is limited to `Cargo.toml`, `Cargo.lock`, and the three packaging defaults. A release commit should use the imperative message `chore: bump version to X.Y.Z`.
...
- If the user explicitly asks to commit the version bump, review the exact five-file diff first and commit it with `chore: bump version to X.Y.Z`.
...
- target version and files changed;
