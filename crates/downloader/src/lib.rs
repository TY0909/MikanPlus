...
fn pending_remove(hash: &str) -> bool {
    PENDING
        .lock()
        .unwrap()
        .as_mut()
        .is_some_and(|pending| pending.remove(hash).is_some())
}
        ...
        tokio::select! {
            cmd = cmd_rx.recv() => {
                ...
                if let DownloadCmd::Add { magnet, title, output_dir } = &cmd {
                    // 在启动异步 metadata 获取前同步登记，确保紧随其后的取消或
                    // 目录清理一定能观察到这个任务。
...
async fn handle_cmd(session: &Arc<Session>, meta_dir: &Path, cmd: DownloadCmd) {
    match cmd {
        ...
        DownloadCmd::Cancel { id } => {
            ...
            // 只有 metadata 获取中的任务需要延迟取消标记；已注册任务直接删除。
            if pending_remove(&id) {
                cancelled_insert(&id);
            }
