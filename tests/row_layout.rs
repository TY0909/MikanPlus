...
use gpui::{Context, Render, Size, VisualTestContext, Window, div, prelude::*, px, relative};
...
#[gpui::test]
async fn sixty_forty_wide_window(cx: &mut gpui::TestAppContext) {
    let mut cx = setup(cx, 800., LONG_TITLE, LONG_KEYWORD);
    check(&mut cx, "wide");
}

#[gpui::test]
async fn sixty_forty_narrow_window(cx: &mut gpui::TestAppContext) {
    let mut cx = setup(cx, 420., LONG_TITLE, LONG_KEYWORD);
    check(&mut cx, "narrow");
}

#[gpui::test]
async fn sixty_forty_ascii_title(cx: &mut gpui::TestAppContext) {
