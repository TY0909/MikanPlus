...
/// 按圆角方式给元素设置单边/单角圆角
fn with_corners<T: gpui::Styled>(el: T, corners: CornerStyle, r: f32) -> T {
    match corners {
...
pub fn poster(
    ...
    if path.exists() {
        // 本地资源(缓存文件 / 打包资源)
        with_corners(gpui::img(path), corners, rounded)
            .w(width)
            .h(height)
...
fn lazy_cover(
    ...
    // 已缓存:直接显示
    if let Some(cache_path) = crate::cache::cached_image(&fetch_url) {
        return with_corners(gpui::img(cache_path), corners, rounded)
            .w(width)
            .h(height)
            .object_fit(gpui::ObjectFit::Cover)
            .into_any_element();
        ...
        .child(
            gpui::canvas(
                ...
                move |bounds, _prepaint, window, _app| {
                    ...
                    if bounds.intersects(&visible) && crate::network::claim_image(&url) {
                        let url = url.clone();
                        std::thread::spawn(move || {
                            let ok = crate::network::fetch_bytes(&url)
                                .and_then(|bytes| {
                                    crate::cache::store_image(&url, &bytes)
                                        .map(|_| ())
                                        .ok_or_else(|| "写入缓存失败".to_string())
                                })
                                .is_ok();
                            crate::network::finish_image(&url, ok);
                        });
                    }
                },
            )
...
fn placeholder_inner(
    ...
    rounded: f32,
    corners: CornerStyle,
    fixed: Option<(gpui::Pixels, gpui::Pixels)>,
) -> impl IntoElement {
