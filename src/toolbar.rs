...
use gpui::{App, Context, Entity, Window, prelude::*, px};
...
use crate::navigation::TopSection;
...
pub struct Toolbar {
    ...
    _enter_subscription: gpui::Subscription,
...
impl Render for Toolbar {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let nav_btn = |icon_name: &str, enabled: bool, cb: ActionCallback| {
            let id: gpui::SharedString = format!("tb-{icon_name}").into();
            let mut el = gpui::div()
                .id(id.clone())
                .size(px(28.))
                .rounded(px(6.))
