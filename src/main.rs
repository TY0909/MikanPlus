...
use anyhow::Result;
use gpui::{
    App, Application, AssetSource, Bounds, Context, Entity, ScrollHandle, SharedString, Window,
    WindowBounds, WindowOptions, prelude::*, px,
};
...
use mikan_plus::actions::*;
use mikan_plus::app_theme;
use mikan_plus::data::{
    BangumiItem, Subscription, load_subgroup_keywords, load_subscriptions, save_subgroup_keywords,
    save_subscriptions,
};
use mikan_plus::download::{DownloadCmd, DownloadManager};
use mikan_plus::home_view::{FilterChangeCallback, HomeView, today_weekday};
use mikan_plus::menu::build_menus;
use mikan_plus::navigation::{HomeFilter, Page, TopSection};
use mikan_plus::parser::SearchResults;
use mikan_plus::subgroup_detail_page::OpenFilterCallback;
use mikan_plus::subscription_page::{SubCardClickCallback, UnsubscribeCallback};
use mikan_plus::toolbar::{ActionCallback, NavigateCallback, SearchCallback, Toolbar};
use mikan_plus::{
    BangumiDetailPage, BangumiGroup, SearchResultPage, SettingsPage, SubGroupDetailPage,
    SubscriptionPage,
};
...
impl MikanPlus {
    fn new(window: &mut Window, cx: &mut App) -> Entity<Self> {
        cx.new(|cx: &mut Context<Self>| {
            ...
            // 后台线程执行:涉及目录删除,不阻塞首帧
            std::thread::spawn(|| {
                mikan_plus::migrate::run_all();
            });

            // 图片缓存容量治理(LRU,后台线程,不阻塞 UI)
            std::thread::spawn(|| {
                mikan_plus::cache::enforce_image_cache_limit();
            });

            // 轮询封面图片缓存、后台加载与下载快照版本:变化时刷新界面;
            // 同时消费下载后台事件(失败通知)。
            cx.spawn(
                move |this: gpui::WeakEntity<MikanPlus>, cx: &mut gpui::AsyncApp| {
                    let mut cx = cx.clone();
                    async move {
                        let mut last_img = mikan_plus::network::image_version();
                        let mut last_load = LOAD_VERSION.load(Ordering::Relaxed);
                        let mut last_dl = mikan_plus::download::snapshot_version();
                        loop {
                            cx.background_executor()
                                .timer(std::time::Duration::from_millis(500))
                                .await;
                            let iv = mikan_plus::network::image_version();
                            let lv = LOAD_VERSION.load(Ordering::Relaxed);
                            let dv = mikan_plus::download::snapshot_version();
                            ...
                            // 消费下载后台事件 → 应用内通知 / 退订回执
                            let events = mikan_plus::download::take_events();
                            if !events.is_empty()
                                && let Some(entity) = this.upgrade() {
                                    entity
                                        .update(&mut cx, |mp, cx| {
                                            for e in events {
                                                match e {
                                                    mikan_plus::download::DownloadEvent::AddFailed {
                                                        title,
                                                        error,
                                                    } => {
                                                    ...
                                                    mikan_plus::download::DownloadEvent::EngineFailed {
                                                        error,
                                                    } => {
                                                        let _ = mp.window_handle.update(
                                                            cx,
                                                    ...
                                                    mikan_plus::download::DownloadEvent::UnsubscribeBlocked {
                                                        dir,
                                                        titles,
                                                    } => mp.handle_unsubscribe_blocked(&dir, titles),
                                                    mikan_plus::download::DownloadEvent::UnsubscribeDone {
                                                        dir,
                                                    } => {
            ...
            let on_toggle_theme: ActionCallback = {
                Rc::new(move |window, app: &mut App| {
                    let dark = gpui_component::theme::Theme::global(app).mode
    ...
    fn start_list_load(&mut self, cx: &mut Context<Self>) {
        ...
        // 磁盘缓存
        if let Some(v) = mikan_plus::cache::cached_list()
        ...
        mikan_plus::network::reset_backoff(mikan_plus::network::BASE_URL);
        self.list_state = ListState::Loading;
        std::thread::spawn(|| {
            let result = mikan_plus::network::fetch_html(mikan_plus::network::BASE_URL)
                .map(|html| mikan_plus::parser::parse_bangumi_list(&html));
            if let Ok(groups) = &result {
                // 写缓存,减少后续访问
                if let Ok(v) = serde_json::to_value(groups) {
                    mikan_plus::cache::store_list(&v);
                }
            }
    ...
    fn ensure_detail(&mut self, name: String, cx: &mut Context<Self>) {
        ...
        // 磁盘缓存(24 小时 TTL)
        if let Some(v) = mikan_plus::cache::cached_detail(bid)
        ...
        mikan_plus::network::reset_backoff(&format!(
            "{}/Home/Bangumi/{bid}",
            mikan_plus::network::BASE_URL
        ));
        self.detail_loading.insert(name.clone());
        cx.notify();
        std::thread::spawn(move || {
            let url = format!("{}/Home/Bangumi/{bid}", mikan_plus::network::BASE_URL);
            let result = mikan_plus::network::fetch_html(&url)
                .map(|html| mikan_plus::parser::parse_bangumi_detail(&html))
                .map(|(meta, groups)| {
                    let mut item = base;
                    item.meta = Some(meta);
                    ...
                    if let Ok(v) = serde_json::to_value(&item) {
                        mikan_plus::cache::store_detail(bid, &v);
                    }
                    item
                });
    ...
    fn ensure_search(&mut self, query: String, cx: &mut Context<Self>) {
        ...
        if let Some(v) = mikan_plus::cache::cached_search(&query)
        ...
        // 用户主动搜索/重试:清除退避,立即放行
        mikan_plus::network::reset_backoff(&mikan_plus::network::search_url(&query));
        self.search_loading.insert(query.clone());
        cx.notify();
        std::thread::spawn(move || {
            let url = mikan_plus::network::search_url(&query);
            let result = mikan_plus::network::fetch_html(&url)
                .map(|html| mikan_plus::parser::parse_search_results(&html))
                .inspect(|results| {
                    // 写缓存,减少后续访问
                    if let Ok(v) = serde_json::to_value(results) {
                        mikan_plus::cache::store_search(&query, &v);
                    }
                });
    ...
    /// 结果经 [`mikan_plus::download::DownloadEvent`] 回执:
    /// - 有进行中任务 → 阻断,弹出警告窗口
    /// - 清理完成 → 移除订阅记录与筛选关键词
    fn unsubscribe(&mut self, bangumi_id: u32, subgroup_id: u32, cx: &mut Context<Self>) {
        ...
        let dir = mikan_plus::paths::subgroup_download_dir(
            &mikan_plus::data::load_download_dir(),
            &sub.bangumi_name,
            &sub.group_name,
        );
    ...
    fn handle_unsubscribe_blocked(&mut self, dir: &std::path::Path, titles: Vec<String>) {
        ...
        let expected = mikan_plus::paths::subgroup_download_dir(
            &mikan_plus::data::load_download_dir(),
            bangumi_name,
            group_name,
        );
...
impl Render for MikanPlus {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // 内容区
        let content: gpui::AnyElement = match self.current_page.clone() {
            Page::Home(filter) => {
                // 列表加载状态:加载中 / 失败(重试)/ 就绪
            ...
            Page::SearchResult(query) => {
                ...
                if let Some(results) = self.search_results.get(&query).cloned() {
                    ...
                    let on_page_change: mikan_plus::search_result_page::SearchPageChangeCallback = {
                        let entity = cx.entity().clone();
        ...
        let filter_modal_view: Option<gpui::AnyElement> = {
        ...
        let warning_modal_view: Option<gpui::AnyElement> = {
            let entity = cx.entity().clone();
        ...
        gpui::div()
            ...
            .on_action(cx.listener(|_this, _: &ToggleTheme, window, cx| {
                let dark = gpui_component::theme::Theme::global(cx).mode
                    == gpui_component::theme::ThemeMode::Dark;
                let mode = if dark {
                    gpui_component::theme::ThemeMode::Light
                } else {
            ...
            .on_action(cx.listener(|_this, _: &LightMode, window, cx| {
                app_theme::set_mode(gpui_component::theme::ThemeMode::Light, Some(window), cx);
            }))
            .on_action(cx.listener(|_this, _: &DarkMode, window, cx| {
                app_theme::set_mode(gpui_component::theme::ThemeMode::Dark, Some(window), cx);
            }))
...
fn scroll_page(page: impl IntoElement, handle: &ScrollHandle) -> gpui::AnyElement {
    mikan_plus::layout::page_scroll(mikan_plus::layout::MAX_PAGE_W, handle, page).into_any_element()
}
            ...
            gpui::div()
                ...
                .child(
                    gpui::div()
                        ...
                        .items_center()
                        .gap(px(8.))
                        .child(mikan_plus::icons::icon("info", 16.).text_color(theme.warning))
                        .child(
                            gpui::div()
                                .text_base()
                                .font_semibold()
...
fn render_filter_modal(
    ...
    let btn = |label: &str, primary: bool, cb: GoBackCallback| {
        gpui::div()
            ...
            .font_semibold()
            .cursor_pointer()
            .id(gpui::SharedString::from(format!("filter-btn-{label}")))
            .when(primary, |this| {
    ...
    gpui::div()
        ...
        .child(
            gpui::div()
                ...
                .child(gpui::div().mt(px(14.)).child(Input::new(&input).w_full()))
                .child(
                    gpui::div()
                        .mt(px(18.))
                        .flex()
...
/// 加载中占位视图
fn loading_view(theme: &gpui_component::theme::Theme) -> gpui::Div {
    gpui::div()
        .size_full()
        .flex()
        .items_center()
...
/// 加载失败视图:错误信息 + 重试按钮
fn error_view(msg: &str, retry: GoBackCallback, theme: &gpui_component::theme::Theme) -> gpui::Div {
...
fn open_main_window(cx: &mut App) {
    cx.open_window(
        ...
        |window, cx| {
            ...
            cx.new(|cx| gpui_component::Root::new(mikan, window, cx))
        },
    )
    .ok();
}
...
impl AssetSource for Assets {
    fn load(&self, path: &str) -> Result<Option<Cow<'static, [u8]>>> {
        fs::read(self.base.join(path))
            ...
            .map_err(|e| anyhow::anyhow!("加载资源失败 {}: {}", path, e))
    }
    ...
    fn list(&self, path: &str) -> Result<Vec<SharedString>> {
        fs::read_dir(self.base.join(path))
            ...
            .map_err(|e| anyhow::anyhow!("列举资源失败 {}: {}", path, e))
    }
}
