...
use gpui::{App, ScrollHandle, Window, prelude::*, px, relative};
use gpui_component::ActiveTheme;
use gpui_component::StyledExt;
use gpui_component::scroll::ScrollableElement;

use crate::app_theme;
use crate::data::BangumiItem;
use crate::download::{DownloadManager, TaskView};
...
/// 打开外部链接:仅允许 http/https(防止 file://、smb:// 等自定义 scheme 被执行)
fn open_url(url: &str) {
...
impl RenderOnce for BangumiDetailPage {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let dl_base = crate::data::load_download_dir();
        ...
        let link_btn = |label: &str, icon_name: &str, url: &str| {
            ...
            gpui::div()
                ...
                .cursor_pointer()
                .id(gpui::SharedString::from(format!("link-{label}")))
        ...
        let hero_bg = gpui::hsla(hue_a / 360.0, 0.45, if is_dark { 0.16 } else { 0.94 }, 1.0);
        let hero_glow = gpui::hsla(hue_b / 360.0, 0.5, if is_dark { 0.14 } else { 0.9 }, 1.0);

        // 链接行:官方网站 / 番组计划(简洁链接,与上方信息合并)
        let link_row = gpui::div()
            .mt(px(10.))
            .flex()
            .items_center()
            .gap(px(18.))
        ...
        gpui::div()
            ...
            .child(
                gpui::div()
                    ...
                    .child(
                    gpui::div().w_full().flex().flex_col().items_center().child(
                        gpui::div()
                            .w_full()
                            ...
                            .child(
                                ...
                                gpui::div()
                                    ...
                                    .relative()
                                    .child(gpui::div().absolute().inset_0().bg(hero_bg))
                                    .child(
                                        gpui::div()
                                            .absolute()
                                            .right(px(-60.))
...
fn render_group_card(
    gix: usize,
    group_name: String,
    episodes: Vec<crate::data::Episode>,
    subscribed: bool,
    ...
    let subscribe_btn = gpui::div()
        ...
        .flex_shrink_0()
        .cursor_pointer()
        .id(gpui::SharedString::from(format!("grp-sub-{gix}")))
    ...
    let mut name_el = gpui::div()
        .id(gpui::SharedString::from(format!("grp-name-{gix}")))
        .text_base()
        .font_semibold()
        .text_color(theme.foreground)
        .truncate();
    ...
    let rows = episodes.into_iter().enumerate().map(move |(ep_ix, ep)| {
        ...
        gpui::div()
            .id(gpui::SharedString::from(format!("ep-row-{gix}-{ep_ix}")))
            .w_full()
            .px(px(14.))
            .py(px(14.))
            .flex()
