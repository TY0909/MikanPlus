...
use crate::download::{DownloadCmd, DownloadManager, TaskState, TaskView, magnet_info_hash};
...
pub fn title_cell(
    ...
    let mut el = gpui::div()
        .id(gpui::SharedString::from(format!("ep-title-{gix}-{full}")))
        .w_full()
        .text_sm()
...
pub fn action_button(
    ...
    let dl_btn = || {
        ...
        gpui::div()
            ...
            .text_color(theme.foreground)
            .cursor_pointer()
            .id(gpui::SharedString::from(format!("ep-dl-{gix}-{title}")))
    ...
    match task.map(|t| &t.state) {
        Some(TaskState::Completed) => {
            ...
            let open_id = gpui::SharedString::from(format!("ep-open-{gix}-{title}"));
            let more_id = gpui::SharedString::from(format!("ep-more-{gix}-{title}"));
            gpui::div()
                .flex()
                .items_center()
                .gap(px(4.))
        ...
        Some(TaskState::Initializing) | Some(TaskState::Downloading) => {
            ...
            let state_txt = if matches!(task.state, TaskState::Initializing) {
                ...
                format!(
                    "{} · {}源",
                    crate::download::format_percent(task.progress),
                    task.peers
                )
            };
            gpui::div()
    ...
    gpui::div()
        ...
        .child(
            gpui::div()
                ...
                .child(crate::download::format_rate(rate)),
        )
}
...
fn cancel_btn(
    ...
    gpui::div()
        ...
        .text_color(theme.muted_foreground)
        .cursor_pointer()
        .id(gpui::SharedString::from(format!("ep-cancel-{gix}-{title}")))
