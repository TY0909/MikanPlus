...
impl RenderOnce for SubGroupDetailPage {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let dl_dir = paths::subgroup_download_dir(
        ...
        let visible: Vec<(usize, crate::data::Episode)> = episodes
            .into_iter()
            .enumerate()
            .filter(|(_, ep)| {
        ...
        let filter_btn = {
            ...
            gpui::div()
                ...
                .child(
                    ...
                    gpui::div().max_w(px(180.)).truncate().child(if filtering {
                        format!("筛选 · {keyword}")
                    } else {
                        "筛选".to_string()
                    }),
        ...
        let rows = visible.into_iter().map(move |(ix, ep)| {
            ...
            gpui::div()
                .id(gpui::SharedString::from(format!("sg-row-{ix}")))
                .w_full()
                .px(px(14.))
                .py(px(14.))
                .flex()
